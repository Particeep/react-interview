...
class MainContent extends React.Component<MyProps, MyState> {
  ...
  deleteCard(id: string) {
    ...
    if (
      updatedMovies.filter(movie => movie.category === movieCategory).length ===
      0
    ) {
      updatedCategories = this.state.selectedCategories.filter(
        cat => cat !== movieCategory
      );
    }
    ...
    this.setState(prevState => {
      return {
        ...
        selectedCategories:
          ...
          this.filterMovies(prevState.selectedCategories, updatedMovies)
            .length === 0
            ? updatedCategories
  ...
  render() {
    const movieCards = this.filterMovies(
