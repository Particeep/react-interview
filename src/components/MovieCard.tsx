      ...
      <div className='movie-card'>
        <div id='card-infos'>
          ...
          <button
            ...
            onClick={ () => this.props.deleteCard(this.props.id) }>Delete</button>
        </div>
        <div id='like-infos'>
          <div id='like-count'>
            <p>{ likes }</p>
          ...
          <div id='like-bar'>
            <FontAwesome className='thumbs-up' name='thumbs-up'
              style={ { fontSize: this.props.isLiked ? '20px' : '15px' } }
              onClick={ () => this.props.updateLike(this.props.id, this.props.isLiked ? false : true) } />
            <div id='likes'
              style={ { width:  likeWidth } }></div>
            <div id='dislikes' style={ { width: dislikeWidth } }></div>
            <FontAwesome className='thumbs-down' name='thumbs-down'
              style={ { fontSize: this.props.isDisliked ? '20px' : '15px' } }
              onClick={ () => this.props.updateDislike(this.props.id, this.props.isDisliked ? false : true) } />
          </div>
        </div>
      </div>
    )
